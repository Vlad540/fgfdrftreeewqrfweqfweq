...
public class WebRtcAudioTrack {
    ...
    private void logMainParameters() {
        ...
        StringBuilder i = rf0.i("AudioTrack: session ID: ", audioSessionId, ", channels: ", channelCount, ", sample rate: ");
        i.append(sampleRate);
        i.append(", max gain: ");
        i.append(maxVolume);
        ...
        if (!TRACK_LATEINIT) {
            try {
                ...
                if (this.audioTrack.getPlayState() != 3) {
                    reportWebRtcAudioTrackStartError(JavaAudioDeviceModule.AudioTrackStartErrorCode.AUDIO_TRACK_START_STATE_MISMATCH, wn6.h(this.audioTrack.getPlayState(), "AudioTrack.play failed - incorrect state :"));
                    releaseAudioResources();
                    return false;
                }
            } catch (IllegalStateException e) {
                reportWebRtcAudioTrackStartError(JavaAudioDeviceModule.AudioTrackStartErrorCode.AUDIO_TRACK_START_EXCEPTION, rf0.h("AudioTrack.play failed: ", e.getMessage()));
                releaseAudioResources();
                return false;
            }
        }
