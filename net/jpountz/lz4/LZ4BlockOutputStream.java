...
public class LZ4BlockOutputStream extends FilterOutputStream {
    ...
    private static int compressionLevel(int i) {
        if (i < MIN_BLOCK_SIZE) {
            throw new IllegalArgumentException(wn6.h(i, "blockSize must be >= 64, got "));
        } else if (i <= MAX_BLOCK_SIZE) {
            return Math.max(0, 22 - Integer.numberOfLeadingZeros(i - 1));
        } else {
            throw new IllegalArgumentException(wn6.h(i, "blockSize must be <= 33554432, got "));
        }
    }
    ...
    private void flushBufferedData() throws IOException {
        ...
        if (this.o != 0) {
            ...
            if (compress >= i4) {
                ...
                i = COMPRESSION_METHOD_RAW;
            } else {
                i4 = compress;
                i = COMPRESSION_METHOD_LZ4;
            }
    ...
    private static void writeIntLE(int i, byte[] bArr, int i2) {
        bArr[i2] = (byte) i;
        bArr[i2 + 1] = (byte) (i >>> 8);
        bArr[i2 + 2] = (byte) (i >>> COMPRESSION_METHOD_RAW);
        bArr[i2 + 3] = (byte) (i >>> 24);
    }
    ...
    public void finish() throws IOException {
        ...
        int i = MAGIC_LENGTH;
        bArr[i] = (byte) (this.compressionLevel | COMPRESSION_METHOD_RAW);
