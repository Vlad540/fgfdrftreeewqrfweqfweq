package defpackage;

/* renamed from: qd6  reason: default package */
public abstract /* synthetic */ class qd6 {
    public static final /* synthetic */ int[] a;
    public static final /* synthetic */ int[] b;

    /* JADX WARNING: Can't wrap try/catch for region: R(22:0|(2:1|2)|3|5|6|7|(2:9|10)|11|13|14|15|16|17|19|20|21|22|23|24|25|26|28) */
    /* JADX WARNING: Can't wrap try/catch for region: R(23:0|1|2|3|5|6|7|(2:9|10)|11|13|14|15|16|17|19|20|21|22|23|24|25|26|28) */
    /* JADX WARNING: Can't wrap try/catch for region: R(24:0|1|2|3|5|6|7|9|10|11|13|14|15|16|17|19|20|21|22|23|24|25|26|28) */
    /* JADX WARNING: Code restructure failed: missing block: B:29:?, code lost:
        return;
     */
    /* JADX WARNING: Failed to process nested try/catch */
    /* JADX WARNING: Missing exception handler attribute for start block: B:15:0x001c */
    /* JADX WARNING: Missing exception handler attribute for start block: B:21:0x002c */
    /* JADX WARNING: Missing exception handler attribute for start block: B:23:0x0030 */
    /* JADX WARNING: Missing exception handler attribute for start block: B:25:0x0034 */
    static {
        /*
            w8f[] r0 = defpackage.w8f.values()
            int r0 = r0.length
            int[] r0 = new int[r0]
            b = r0
            r1 = 0
            r2 = 1
            r0[r1] = r2     // Catch:{ NoSuchFieldError -> 0x000d }
        L_0x000d:
            r0 = 2
            int[] r3 = b     // Catch:{ NoSuchFieldError -> 0x0012 }
            r3[r2] = r0     // Catch:{ NoSuchFieldError -> 0x0012 }
        L_0x0012:
            r3 = 3
            int[] r4 = b     // Catch:{ NoSuchFieldError -> 0x0017 }
            r4[r3] = r3     // Catch:{ NoSuchFieldError -> 0x0017 }
        L_0x0017:
            r4 = 4
            int[] r5 = b     // Catch:{ NoSuchFieldError -> 0x001c }
            r5[r4] = r4     // Catch:{ NoSuchFieldError -> 0x001c }
        L_0x001c:
            int[] r5 = b     // Catch:{ NoSuchFieldError -> 0x0021 }
            r6 = 5
            r5[r0] = r6     // Catch:{ NoSuchFieldError -> 0x0021 }
        L_0x0021:
            k88[] r5 = defpackage.k88.values()
            int r5 = r5.length
            int[] r5 = new int[r5]
            a = r5
            r5[r1] = r2     // Catch:{ NoSuchFieldError -> 0x002c }
        L_0x002c:
            int[] r1 = a     // Catch:{ NoSuchFieldError -> 0x0030 }
            r1[r2] = r0     // Catch:{ NoSuchFieldError -> 0x0030 }
        L_0x0030:
            int[] r1 = a     // Catch:{ NoSuchFieldError -> 0x0034 }
            r1[r0] = r3     // Catch:{ NoSuchFieldError -> 0x0034 }
        L_0x0034:
            int[] r0 = a     // Catch:{ NoSuchFieldError -> 0x0038 }
            r0[r3] = r4     // Catch:{ NoSuchFieldError -> 0x0038 }
        L_0x0038:
            return
        */
        throw new UnsupportedOperationException("Method not decompiled: defpackage.qd6.<clinit>():void");
    }
}
